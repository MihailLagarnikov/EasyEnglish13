...
class MyLessonPresenter(val mModel: MyViewModel): ContractMVP.LessonPresenter {
    ...
    override fun getFragmentTestType(): Fragment {
        if (!mLessonDone) {
           ...
           return getRandomTestType(getRandomLessonWord())
            ...
            InnerData.saveBoolean(LESSON_USALI_ALRM + mThemeName,false)
    ...
    override fun getTestDate(typeTest: TypeTest): DataMidleFragment {
        mTypeTest=typeTest
        when (typeTest){
            TypeTest.TestA -> return createRandomLessonADate(createListTrueAndFalsWords(mEnWards.trim(),mRuWards.trim(),true))
            TypeTest.TestB -> return createRandomLessonADate(createListTrueAndFalsWords(mRuWards.trim(),mEnWards.trim(),false))
            TypeTest.TestC -> return DataMidleFragment(mRuWards.trim(), titleEn = mEnWards.trim())
            TypeTest.TestD -> return DataMidleFragment(mEnWards.trim(), titleEn = mRuWards.trim())
            TypeTest.TestE -> return DataMidleFragment(mRuWards.trim(), titleEn = mEnWards.trim())
        }
    }
